...
function process_duplicates($contextId = 0, $courseId = 0, $categoryId = 0, $force = false, $dryRun = false, $limit = 10000000, $contextAware = true) {
    ...
    $duplicateGroups = array_filter($questionsByStamp, function($group) {
    ...
    $duplicatesToUpdate = array_filter($duplicates, function($dup) {
    ...
    foreach ($contextDuplicates as $contextData) {
        ...
        $dupsToUpdateInContext = count(array_filter($contextData['duplicates'], function($dup) {
            return $dup->row_num > 1;
        }));
...
try {
    list($duplicateCount, $updatedCount) = process_duplicates($contextId, $courseId, $categoryId, $force, $dryRun, $limit, $contextAware);
